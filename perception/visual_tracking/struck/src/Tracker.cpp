...
Tracker::Tracker(const Config& conf)
    : m_config(conf),
      m_initialised(false),
      m_pLearner(0),
      m_debugImage(2 * conf.searchRadius + 1, 2 * conf.searchRadius + 1,
                   CV_32FC1),
...
void Tracker::UpdateDebugImage(const vector<FloatRect>& samples,
  ...
  for (int i = 0; i < (int)samples.size(); ++i) {
    ...
    m_debugImage.at<float>(m_config.searchRadius + y,
                           m_config.searchRadius + x) =
        (float)((scores[i] - mn) / (mx - mn));
  }
}
...
void Tracker::UpdateLearner(const ImageRep& image) {
  ...
  vector<FloatRect> rects =
      Sampler::RadialSamples(m_bb, 2 * m_config.searchRadius, 5, 16);
  // vector<FloatRect> rects = Sampler::PixelSamples(m_bb,
  // 2*m_config.searchRadius, true);
