...
class ImageRep {
 public:
  ImageRep(const cv::Mat& rImage, bool computeIntegral,
           bool computeIntegralHists, bool colour = false);

  int Sum(const IntRect& rRect, int channel = 0) const;
  void Hist(const IntRect& rRect, Eigen::VectorXd& h) const;

  inline const cv::Mat& GetImage(int channel = 0) const {
    return m_images[channel];
  }
