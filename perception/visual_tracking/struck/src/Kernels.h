...
class LinearKernel : public Kernel {
 public:
  inline double Eval(const Eigen::VectorXd& x1,
                     const Eigen::VectorXd& x2) const {
    return x1.dot(x2);
  }

  inline double Eval(const Eigen::VectorXd& x) const { return x.squaredNorm(); }
};
...
class GaussianKernel : public Kernel {
 public:
  ...
  inline double Eval(const Eigen::VectorXd& x) const { return 1.0; }
...
class IntersectionKernel : public Kernel {
 public:
  ...
  inline double Eval(const Eigen::VectorXd& x) const { return x.sum(); }
};
 ...
 public:
  ...
  inline double Eval(const Eigen::VectorXd& x1,
    ...
    for (int i = 0; i < m_n; ++i) {
      int c = m_counts[i];
      sum += m_norm *
             m_kernels[i]->Eval(x1.segment(start, c), x2.segment(start, c));
      start += c;
    }
    return sum;
  }
