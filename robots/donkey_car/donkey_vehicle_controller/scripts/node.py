...
class Controller:
    ...
    def update(self, cmd):
        if cmd < 0:
            pulse = map_index(cmd, self.RANGE_MIN, 0, self.params['min_pulse'], self.params['zero_pulse'])
        else:
            pulse = map_index(cmd, 0, self.RANGE_MAX, self.params['zero_pulse'], self.params['max_pulse'])
...
if __name__ == '__main__':
    ...
    rospy.Subscriber(node_params.lat_lon_command_topic, LatLonCommand, handler.callback)
    rospy.spin()
